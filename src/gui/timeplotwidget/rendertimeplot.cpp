...
RenderTimeplot::RenderTimeplot( int width, int height, QWidget *parent )
            : QWidget( parent )
{
    m_width  = width;
    m_height = height;
...
QSize RenderTimeplot::minimumSizeHint() const  {  return QSize( m_width, m_height );  }

QSize RenderTimeplot::sizeHint() const  { return QSize( m_width, m_height ); }

void RenderTimeplot::setMaxMin( double max, double min )
{
    m_vMax = max;
    m_vMin = min;
}
void RenderTimeplot::setData( int data[] )
{
    m_data = data;
    update();
}

void RenderTimeplot::paintEvent( QPaintEvent* /* event */ )
{
    QPainter p( this );
